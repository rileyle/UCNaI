...
if(len(sys.argv) == 4):
    ...
    print('Usage: python plotPCA.py <filename1> <filename2> <binning=1>')
    exit

_, _, bins1, counts1 = readMca(sys.argv[1], binning)
_, _, bins2, counts2 = readMca(sys.argv[2], binning)

plt.step(bins1, counts1, where='mid', color='black', label=sys.argv[1])
plt.step(bins2, counts2, where='mid', color='blue', label=sys.argv[2])
#plt.yscale('log')
plt.xlabel('E (keV)')
